...
t.test(atividades_paradigmas$n, as.numeric(atividades_paradigmas$paradigma))
...
kruskal.test(atividades_paradigmas$n_acertos, as.factor(atividades_paradigmas$paradigma))
kruskal.test(atividades_paradigmas$coeficiente, as.factor(atividades_paradigmas$paradigma))
...
procedural <- atividades_paradigmas %>%
                filter(paradigma %in% c("Procedural"))
...
```{r}
wilcox.test(procedural$n, funcional$n)
wilcox.test(procedural$n_acertos, funcional$n_acertos)
wilcox.test(procedural$coeficiente, funcional$coeficiente)
...
wilcox.test(procedural$n, logico$n)
wilcox.test(procedural$n_acertos, logico$n_acertos)
wilcox.test(procedural$coeficiente, logico$coeficiente)
```
...
wilcox.test(funcional$n, logico$n)
wilcox.test(funcional$n_acertos, logico$n_acertos)
wilcox.test(funcional$coeficiente, logico$coeficiente)
...
###  Tempo por atividades por alunos

```{r}
alunos_atividades <- dados_plp %>%
  group_by(cod_atividade, aluno) %>%
  count(cod_atividade)

teste_atv_alunos <- dados_plp %>%
  group_by(cod_atividade, aluno) %>% count(correta)

acertos_atividade_aluno <- teste_atv_alunos %>%
  filter(correta == TRUE)
  
erros_atividade_aluno <- teste_atv_alunos %>%
  filter(correta == FALSE) 

dados_atividade_alunos <- merge(
  x=acertos_atividade_aluno,
  y=erros_atividade_aluno[,c("cod_atividade", "n")],
  by = "cod_atividade",
  suffix = c("_acertos", "_erros"),
  all.x = TRUE
)
#dados_atividade_alunos <- merge(
#  x=dados_atividade_alunos,
#  y=submissoes_atv, by = "cod_atividade", all.x = TRUE) %>% 
#  mutate(coeficiente = n_acertos/n) %>% 
#  select(-correta)
```
